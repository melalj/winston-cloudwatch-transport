...
var Transport = function (options) {
    ...
    this.jsonMessage = typeof(options.jsonMessage) !== 'undefined' ? options.jsonMessage : true;
    ...
    this.add = function (message) {
        var messageSize;
        if (this.jsonMessage) {
            messageSize = CircularJSON.stringify(message).length;
        } else {
...
Transport.prototype.log = function (level, msg, meta, callback) {
    var message;
    if (this.jsonMessage) {
      message = CircularJSON.stringify({level: level, msg: msg, meta: meta}, null, '  ');
    } else {
      message = level + ': ' + msg;
    }
    this.add({message: message, timestamp: Date.now()});
